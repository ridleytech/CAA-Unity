...
using Opsive.UltimateCharacterController.Networking.Game;
    ...
    public class ShootableWeapon : UsableItem, IReloadableItem
    {
                ...
                if (damageAmount > 0) {
                    ...
                    if ((hitHealth = hitGameObject.GetCachedParentComponent<Health>()) != null) {
                        ...
                        if (m_NetworkInfo != null) {
                            // All clients will receive the Fire RPC. Local players should send the hitscan damage RPC so the target is always hit no matter the amount of lag.
                            if (m_NetworkInfo.IsLocalPlayer()) {
                                m_NetworkCharacter.ShootableHitscanDamage(this, hitHealth.gameObject, damageAmount, closestRaycastHit.point, fireDirection, strength, closestRaycastHit.collider);
                            }
                        } else {
#endif
                            HitscanDamage(hitHealth, damageAmount, closestRaycastHit.point, fireDirection, strength, closestRaycastHit.collider);
#if ULTIMATE_CHARACTER_CONTROLLER_MULTIPLAYER
                        }
#endif
        ...
        /// <summary>
        /// A hitscan has caused damage.
        /// </summary>
        /// <param name="hitHealth">The health component that was damaged.</param>
        ...
        public void HitscanDamage(Health hitHealth, float damageAmount, Vector3 position, Vector3 direction, float strength, Collider hitCollider)
        {
            if (hitHealth.CanDamage(damageAmount)) {
                hitHealth.OnDamage(damageAmount, position, direction, m_ImpactForce * strength, m_ImpactForceFrames, 0, m_Character, this, hitCollider);
            }
        }
